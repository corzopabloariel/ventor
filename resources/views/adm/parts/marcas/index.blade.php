...
<script>
    ...
    window.pyrus = new Pyrus("marcas", null, src);
    window.modelos = new Pyrus("modelos", null, src);
    window.elementos = @json($marcas);
    ...
    addModal = function(t, id = 0, data = null) {
        ...
        if(id != 0)
            action = `{{ url('/adm/${window.pyrus.entidad}/${window.modelos.entidad}/update/${id}') }}`;
        else
            action = `{{ url('/adm/${window.pyrus.entidad}/${window.modelos.entidad}/store') }}`;
        if(data !== null) {
            
            for(let x in window.modelos.especificacion) {
    ...
    removeModal = function(t) {
        ...
        for(let x in window.modelos.especificacion) {
            if(window.modelos.especificacion[x].EDITOR !== undefined) {
                CKEDITOR.instances[x].setData('');
                continue;
            }
            if(window.modelos.especificacion[x].TIPO == "TP_FILE")
                $(`#src-${x}_sub`).attr("src","");
            $(`[name="${x}_sub"]`).val("");
        }
    };
    ...
    editModal = function(t, id) {
        $(t).attr("disabled",true);
        let promise = new Promise(function (resolve, reject) {
            let url = `{{ url('/adm/${window.pyrus.entidad}/${window.modelos.entidad}/edit/${id}') }}`;
            var xmlHttp = new XMLHttpRequest();
            xmlHttp.responseType = 'json';
            xmlHttp.open( "GET", url, true );
            xmlHttp.onload = function() {
    ...
    submitModal = function(t) {
        ...
        xmlHttp.onload = function() {
            ...
            if(table.find(`tbody tr[data-id="${xmlHttp.response.id}"]`).length) {
                ...
                let columnas = window.modelos.columnas();
                tr = "";
                ...
                columnas.forEach(function(c) {
                    ...
                    if(typeof td == 'object')
                        td = td.nombre;
                    if(window.modelos.especificacion[c.COLUMN].TIPO == "TP_FILE") {
                        date = new Date();
        ...
        promiseFunction = () => {
            promise
                .then(function(data) {
                    ...
                    $("#formModal .container-formModal").html(window.modelos.formulario("sub"));
